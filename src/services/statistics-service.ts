...
export class StatisticsService {
    static async categoryPercentages() {
        try {
            ...
            const categoryPercentages = countComplaintsPerCategory.map((category) => ({
                _id: category._id,
                name: categoryResponses.find((categoryResponse) => categoryResponse._id === category._id.toString())?.name || 'Unknown',
                has_complaints: category.count,
                percentage: ((category.count / totalComplaints) * 100).toFixed(1).concat('%'),
            }));
            ...
            throw new CustomErrors(500, 'Internal Server Error', 'An error ocurred while count complaint percentages per category');
        }        
    }
